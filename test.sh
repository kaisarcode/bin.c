#!/bin/sh
# test.sh
# Summary: Validation suite for bin functionality.
# Author:  KaisarCode
...
# Tests that the binary exits 0 on valid input.
# @return 0 on success, 1 on failure.
kc_test_basic_exec() {
    if ! "$BIN" run "test-input" -p "val" > /dev/null 2>&1; then
...
# Tests that the binary accepts input from standard input.
# @return 0 on success, 1 on failure.
kc_test_stdin() {
    if ! printf "test-input" | "$BIN" run -p "val" > /dev/null 2>&1; then
        kc_test_fail "stdin execution"
        return 1
    fi

    kc_test_pass "stdin execution"
}
